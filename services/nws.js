...
const nwsRequest = async (url, opts, sendJSON = true) =>
  await request(url, { headers: HEADERS, ...opts }, sendJSON);
...
class NWS {
  ...
  getNearbyStations = async (url) =>
    await this.getData("stations", url, async () => await nwsRequest(url));
  ...
  getForecastOffice = async ({ lat, lon }) => {
    try {
      const points = await this.getPoints({ lat, lon });
      const { observationStations: stationsUrl } = points;
      const { stationIdentifier, county: countyUrl } =
        await this.getNearestStation(stationsUrl);
  ...
  getClimateReport = async ({ lat, lon }) => {
    try {
      ...
      const url = `https://api.weather.gov/products/types/CLI/locations/${stationIdentifier.slice(
        1
      )}`;
...
const parseGriddedForecastByDay = (grid) => {
  return gridKeys.reduce((parsed, key) => {
    ...
    fullValues.forEach((v, idx) => {
      ...
      const mom = moment(ts);
      const increasingTime = Array.from({ length: duration }, (_, idx) =>
        mom.clone().add(idx, "hours").toISOString()
      );
